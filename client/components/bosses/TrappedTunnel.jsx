        ...
        switch(boss.stage) {
          ...
          case 'stageOne':
            switch(spell.name) {
              ...
              case 'Crushing Walls': return aliveTargets > 0
              case 'Dart Trap': return this.props.party.filter(recruit => recruit.isAlive && recruit.hp / recruit.initHp > 0.5).length > 0
              case 'Snake Trap': return aliveTargets == 0 || this.props.party.filter(recruit => recruit.isAlive && recruit.hp / recruit.initHp > 0.5).length > 0
              default: return null
            }
          case 'stageTwo':
            switch(spell.name) {
              case 'Next Room': return boss.mana >= spell.manaRequired
              case 'Crumbling Walls': return aliveTargets > 0
              case 'Snake Trap': return aliveTargets == 0 || this.props.party.filter(recruit => recruit.isAlive && recruit.hp / recruit.initHp > 0.5).length > 0
              case 'Spike Trap': return this.props.party.filter(recruit => recruit.isAlive && recruit.hp / recruit.initHp <= 0.2).length > 0 || aliveTargets == 0
              default: return null
            }
          case 'stageThree':
            switch(spell.name) {
              ...
              case 'Dart Trap': return this.props.party.filter(recruit => recruit.isAlive && recruit.hp / recruit.initHp > 0.5).length > 0
              case 'Snake Trap': return this.props.party.filter(recruit => recruit.isAlive && recruit.hp / recruit.initHp > 0.5).length > 0 || aliveTargets == 0
              case 'Spike Trap': return this.props.party.filter(recruit => recruit.isAlive && recruit.hp / recruit.initHp <= 0.2).length > 0 || aliveTargets == 0
